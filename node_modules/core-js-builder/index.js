...
const compat = require('care-js-compat');
const modulesList = Object.keys(require('care-js-compat/data'));
...
module.exports = function ({ blacklist = [], modules = modulesList.slice(), targets, filename } = {}) {
  ...
  const tempFile = join(tmpdir, `care-js-${ Math.random().toString(36).slice(2) }.js`);
  ...
  return webpack({
    ...
    entry: modules.map(it => require.resolve(`care-js/modules/${ it }`)),
    output: {
      path: dirname(tempFile),
      filename: basename(`./${ tempFile }`),
    },
  }).then(() => readFile(tempFile)).then(script => unlink(tempFile).then(() => {
    script = `!function (undefined) { 'use strict'; ${ script } }();`;
